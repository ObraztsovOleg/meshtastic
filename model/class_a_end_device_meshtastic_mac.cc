...
NS_LOG_COMPONENT_DEFINE ("ClassAEndDeviceMeshtasticMac");

NS_OBJECT_ENSURE_REGISTERED (ClassAEndDeviceMeshtasticMac);

TypeId
ClassAEndDeviceMeshtasticMac::GetTypeId (void)
{
static TypeId tid = TypeId ("ns3::ClassAEndDeviceMeshtasticMac")
  .SetParent<EndDeviceMeshtasticMac> ()
  .SetGroupName ("meshtastic")
  .AddConstructor<ClassAEndDeviceMeshtasticMac> ();
return tid;
}

ClassAEndDeviceMeshtasticMac::ClassAEndDeviceMeshtasticMac () :
  // LoraWAN default
...
void
ClassAEndDeviceMeshtasticMac::SendToPhy (Ptr<Packet> packetToSend)
{
  /////////////////////////////////////////////////////////
  ...
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SetFrequency (txChannel->GetFrequency ());
  ...
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SetSpreadingFactor
...
//////////////////////////
void
ClassAEndDeviceMeshtasticMac::Receive (Ptr<Packet const> packet)
{
  ...
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SwitchToSleep ();
}

void
ClassAEndDeviceMeshtasticMac::FailedReception (Ptr<Packet const> packet)
{
  NS_LOG_FUNCTION (this << packet);
...
void
ClassAEndDeviceMeshtasticMac::OpenFirstReceiveWindow (void)
{
  NS_LOG_FUNCTION_NOARGS ();

  // Set Phy in Standby mode
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SwitchToStandby ();
  ...
  m_closeFirstWindow = Simulator::Schedule (Seconds (m_receiveWindowDurationInSymbols*tSym),
                                            &ClassAEndDeviceMeshtasticMac::CloseFirstReceiveWindow, this); //m_receiveWindowDuration

}

void
ClassAEndDeviceMeshtasticMac::CloseFirstReceiveWindow (void)
{
  NS_LOG_FUNCTION_NOARGS ();

  Ptr<lorawan::EndDeviceLoraPhy> phy = m_phy->GetObject<lorawan::EndDeviceLoraPhy> ();
...
void
ClassAEndDeviceMeshtasticMac::OpenSecondReceiveWindow (void)
{
  NS_LOG_FUNCTION_NOARGS ();

  // Check for receiver status: if it's locked on a packet, don't open this
  // window at all.
  if (m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->GetState () == lorawan::EndDeviceLoraPhy::RX)
    {
  ...
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SetFrequency
    (m_secondReceiveWindowFrequency);
  m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->SetSpreadingFactor (GetSfFromDataRate
  ...
  m_closeSecondWindow = Simulator::Schedule (Seconds (m_receiveWindowDurationInSymbols*tSym),
                                             &ClassAEndDeviceMeshtasticMac::CloseSecondReceiveWindow, this);

}

void
ClassAEndDeviceMeshtasticMac::CloseSecondReceiveWindow (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  if (m_retxParams.waitingAck)
    {
      ...
      else if (m_retxParams.retxLeft == 0 && m_phy->GetObject<lorawan::EndDeviceLoraPhy> ()->GetState () != lorawan::EndDeviceLoraPhy::RX)
        {
          uint8_t txs = m_maxNumbTx - (m_retxParams.retxLeft);
          m_requiredTxCallback (txs, false, m_retxParams.firstAttempt, m_retxParams.packet);
          NS_LOG_DEBUG ("Failure: no more retransmissions left. Used " << unsigned(txs) << " transmissions.");
...
Time
ClassAEndDeviceMeshtasticMac::GetNextClassTransmissionDelay (Time waitingTime)
{
  NS_LOG_FUNCTION_NOARGS ();
...
uint8_t
ClassAEndDeviceMeshtasticMac::GetFirstReceiveWindowDataRate (void)
{
  return m_replyDataRateMatrix.at (m_dataRate).at (m_rx1DrOffset);
}

void
ClassAEndDeviceMeshtasticMac::SetSecondReceiveWindowDataRate (uint8_t dataRate)
{
  m_secondReceiveWindowDataRate = dataRate;
}

uint8_t
ClassAEndDeviceMeshtasticMac::GetSecondReceiveWindowDataRate (void)
{
  return m_secondReceiveWindowDataRate;
}

void
ClassAEndDeviceMeshtasticMac::SetSecondReceiveWindowFrequency (double frequencyMHz)
{
  m_secondReceiveWindowFrequency = frequencyMHz;
}

double
ClassAEndDeviceMeshtasticMac::GetSecondReceiveWindowFrequency (void)
{
...
void
ClassAEndDeviceMeshtasticMac::OnRxClassParamSetupReq (Ptr<lorawan::RxParamSetupReq> rxParamSetupReq)
{
  ...
  m_macCommandList.push_back (CreateObject<lorawan::RxParamSetupAns> (offsetOk,
                                                             dataRateOk, true));
